...
int load_libc(char *libc_path) {
    ...
    const char *Elf64_Ehdr_chunk;
    const char *memchunk;
    int fd;
    struct stat sb;
    
    if(libc_path == NULL)
        fd = open(RELATIVE_PTH, O_RDONLY);
    else
    ...
    if(fd < 0) {
        if(libc_path != NULL) {
            ...
            if(fd < 0) {
                printf("\033[1;31m[-] '/usr/bin/lib/libc.so' or './lib/libc.so' not found. Skipping FORTIFY checks then...\033[0m\n\n");
                fortify_flag = 0;
            }
    ...
    if(fortify_flag) {
	    ...
	    libc_ehdr = (Elf64_Ehdr *)Elf64_Ehdr_chunk;
        ...
        if(shdr[i].sh_type == SHT_DYNSYM) {
            ...
            for(int j = 0; j < num; j++) {
                ...
                if(sdata + sym[j].st_name == NULL || 
                   !is_pointer_valid(sdata + sym[j].st_name))
                    continue;
    ...
    for (int i = 0; i < elf_ehdr->e_phnum; i++) {
        for (int x = 0; x < libc_ehdr->e_phnum; x++) {
            ...
            if(shdr[i].sh_type == SHT_DYNSYM && libc_shdr[x].sh_type == SHT_DYNSYM) {
                ...
                Elf64_Sym *libc_sym = (Elf64_Sym *)(libc_base + libc_shdr[x].sh_offset);
                ...
                for(int j = 0; j < num; j++) {
                    for(int k = 0; k < libc_num; k++) {
                        ...
                        if(libc_sdata + libc_sym[k].st_name == NULL || 
                           !is_pointer_valid(libc_sdata + libc_sym[k].st_name))
                            continue;
...
void check_interesting_imports(void) {
...
int launch_checks(const char *memchunk, char *file_path, char *libc_path) {
	...
	elf_ehdr = (Elf64_Ehdr *)Elf64_Ehdr_chunk;
    ...
    snprintf(full_arch, sizeof(full_arch)-1, "%s - %s", get_bits(), get_arch());
